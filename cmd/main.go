...
var graphics internal.GraphicTypes

func printHelp() {
	fmt.Printf("Usage: %s gliffy <in.excalidraw> <out.gliffy>\n\n", os.Args[0])
}
...
func main() {
	...
	err := ConvertExcalidrawToGliffy(importPath, exportPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Unable to gliffy Excalidraw diagram to Gliffy diagram: %s\n", err)
		os.Exit(1)
	}
}
...
func ConvertExcalidrawToGliffy(importPath string, exportPath string) error {
	...
	var input internal.ExcalidrawScene
	...
	var output internal.GliffyScene
	var objects []internal.GliffyObject

	graphics := internal.MapGraphics()

	for i, element := range input.Elements {
		var object internal.GliffyObject
		var shape internal.GliffyShape
		var text internal.GliffyText
		var line internal.GliffyLine
	...
	var layer internal.GliffyLayer
	layer.Active = true
...
func OrderGliffyObjectsByPriority(objects []internal.GliffyObject, prioritized []string) []internal.GliffyObject {
